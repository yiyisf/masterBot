...
6. **Slice 5 Conversation Experience short-lived vertical branches (historically “Employee Workspace”)** — private Conversation library and reliable Composer, CMaster-owned Run UI Projection Snapshot/stream, pending governance actions, exact-Version Artifact library, AI Elements presentation, i18n/theme/accessibility, desktop-complete and mobile-core behavior. This Slice is complete; its UI is replaced only after step 7 is authoritative.
7. **Filesystem Workspace short-lived vertical branches** — `packages/workspaces`, private empty/Git Workspace provisioning, one Repository/multiple Worktrees, Revisions, sandboxed read capabilities, immutable Change Sets and Operation Modes, explicit Conversation/Run/Artifact scope, governed Git delivery, and the Balanced Workspace Employee Experience. Follow [`docs/architecture/filesystem-workspace-slice.md`](../architecture/filesystem-workspace-slice.md).
8. **`refactor/production-starter`** — internal OIDC adapter, API/Worker runtime packaging, PostgreSQL plus Artifact/Workspace Content backup/restore, audit and OTel export, security and rate limits, deployment documentation, end-to-end tests, evaluations, and canary acceptance.
