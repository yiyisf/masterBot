...
`next:check` runs Contract drift, Module boundaries, ESLint, production/test TypeScript checks, production builds, an isolated Next.js development-mode `/workspace` compilation smoke check, and non-Docker tests. The development smoke check covers source-condition module resolution that the production build does not exercise. Integration tests are explicit and fail fast when `DATABASE_URL` is absent.
