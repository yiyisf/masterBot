...
记忆管理：
- 当对话较长（>10 轮）或即将切换话题时，主动调用 \`memory_consolidate\` 提炼关键信息，释放上下文窗口。
- 跨会话需要保留的信息用 \`memory_remember\`；本次会话查找历史用 \`memory_recall\`。
...
export const MEMORY_CONSOLIDATE_TOOL: ToolDefinition = {
    type: 'function',
    function: {
        name: 'memory_consolidate',
        description:
            'Compress important facts from the current working memory into long-term Episodic storage. ' +
            'Call this when the conversation is getting long or before switching topics. ' +
            'Saves key decisions, user preferences and findings so they can be recalled in future sessions. ' +
            'After consolidation the context window is trimmed to free up space.',
        parameters: {
            type: 'object',
            properties: {
                key_facts: {
                    type: 'array',
                    items: { type: 'string' },
                    description: 'Important facts, decisions or findings from this conversation to preserve.',
                    minItems: 1,
                },
                topic: {
                    type: 'string',
                    description: 'Short topic tag for the batch (e.g. "database-design", "user-prefs"). Optional.',
                },
                category: {
                    type: 'string',
                    enum: ['user', 'operational', 'governance', 'skill', 'correction', 'reference'],
                    description: 'Memory category (default: operational)',
                },
            },
            required: ['key_facts'],
        },
