...
const HljsHighlighter = makeLightAsyncSyntaxHighlighter({ style: atomOneDark });

const SyntaxHighlighter: FC<
  React.ComponentPropsWithoutRef<typeof HljsHighlighter>
> = (props) => (
  <Suspense
    fallback={
      <pre className="aui-md-pre border-border/50 bg-muted/30 overflow-x-auto rounded-t-none rounded-b-xl border border-t-0 p-3.5 font-mono text-[13px] leading-relaxed">
        {props.code}
      </pre>
    }
  >
    <HljsHighlighter {...props} />
  </Suspense>
);

/** mermaid 代码块直接渲染为流程图 */
const MermaidHighlighter: FC<{ code: string }> = ({ code }) => (
  <div className="my-4">
    <Mermaid code={code} />
  </div>
);
  ...
  return (
    <MarkdownTextPrimitive
      ...
      componentsByLanguage={{
        mermaid: { CodeHeader: () => null, SyntaxHighlighter: MermaidHighlighter },
      }}
      defer
    />
  );
