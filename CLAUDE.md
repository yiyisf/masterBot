...
**核心模块**：
- `src/core/agent.ts` — ReAct 循环编排器，使用 async generator 实现。循环流程：LLM 调用 → 工具调用 → 观察结果 → 重复（最多 10 次迭代）。通过 yield `ExecutionStep` 对象实现流式输出。
- `src/core/database.ts` — SQLite（better-sqlite3），WAL 模式。表：sessions、messages、attachments。数据存储于 `data/cmaster.db`。
...
- `registry.ts` — 已加载技能的中央注册表（Map）。将技能转换为 LLM 工具定义，通过 `SkillContext` 执行动作。

**内存**（`src/memory/short-term.ts`）：基于内存的按会话隔离的键值存储，支持 TTL。`SessionMemoryManager` 管理多个会话。

**网关**（`src/gateway/server.ts`）：Fastify 服务器，提供以下端点：
- `/health` — 健康检查
- `/api/chat` — 非流式聊天
...
**核心集成**：`web/src/lib/assistant-runtime.ts` — 为 @assistant-ui/react 实现的自定义 `ChatModelAdapter`，消费后端 SSE 流（`/api/chat/stream`）。处理 content、thought、action、observation、answer 五种 chunk 类型。
